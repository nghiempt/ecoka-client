...
export const Header = ({
  ...
  useEffect(() => {
    ...
    const fetchNavigations = async () => {
      ...
      if (res && res.data) {
        const visibleNavs = res.data
          .filter((nav: any) => !nav.deleted_at && nav.is_visible)
          .sort((a: any, b: any) => a.order - b.order);
        setNavigations(visibleNavs);
      }
    };
    fetchCategories();
    fetchNavigations();
    ...
    <div className="w-full flex flex-col justify-center items-center">
      ...
      <nav className="w-full hidden lg:flex flex-row justify-center items-center gap-4 py-6 uppercase">
        {navigations.map((nav: any) => {
          const title = lang === "vi" ? nav.title_vi : lang === "en" ? nav.title_en : nav.title_jp;
          
          // Special handling for Products menu with dropdown
          if (nav.path === "/san-pham") {
            return (
              <div key={nav._id} className="relative group h-full">
                <Link
                  href={`/${lang}${nav.path}`}
                ...
                <div className="absolute top-full left-0 flex flex-col gap-3 mt-2 w-64 p-5 pl-7 bg-white opacity-80 text-black shadow-lg rounded-lg transform scale-0 group-hover:scale-100 transition-transform duration-500 ease-in-out z-20">
                  {categories?.map((category: any) => (
                    <Link
                      href={`/${lang}${nav.path}${category.path}`}
                      key={category.name}
          ...
          return (
            <Link
              key={nav._id}
              href={`/${lang}${nav.path}`}
              className="text-[20px] text-[rgb(var(--quaternary-rgb))] font-bold px-4 py-2 rounded-lg hover:bg-[rgb(var(--quaternary-rgb))] hover:opacity-70 hover:text-white"
            >
              {title}
            </Link>
          );
